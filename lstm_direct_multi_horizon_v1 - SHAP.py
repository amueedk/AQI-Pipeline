...
class DirectLSTMMultiHorizon:
    ...
    # ---------- SHAP Explainability ----------
    def run_shap(self, horizons: List[int], tag: str) -> None:
        if not SHAP_AVAILABLE:
            print("⚠️ SHAP not installed; skipping SHAP for LSTM")
            return
        if self._val_X_enc is None or self._val_X_aux is None:
            print("⚠️ No cached validation tensors for SHAP")
            return
        X_enc_val = self._val_X_enc
        X_aux_val = self._val_X_aux
        out_dir = os.path.join(PATHS['logs_dir'], 'shap_lstm')
        os.makedirs(out_dir, exist_ok=True)

        # Build feature names for decoder aux
        aux_feature_names: List[str] = []
        # Realized horizon exogenous templates stored as '{feature}_target_{h}h' with 'h' placeholder
        for t in self.horizon_exogenous_list:
            aux_feature_names.append(t.replace('{h}', 'h'))
        aux_feature_names += ['pos_sin', 'pos_cos', 'pm0_pm2_5_scaled', 'pm0_pm10_scaled']

        # Background and explain samples
        n_bg = min(128, len(X_enc_val))
        n_ex = min(200, len(X_enc_val))
        bg_idx = np.random.RandomState(42).choice(len(X_enc_val), size=n_bg, replace=False)
        ex_idx = np.random.RandomState(123).choice(len(X_enc_val), size=n_ex, replace=False)
        X_enc_bg, X_aux_bg = X_enc_val[bg_idx], X_aux_val[bg_idx]
        X_enc_ex, X_aux_ex = X_enc_val[ex_idx], X_aux_val[ex_idx]

        for h in horizons:
            step_idx = h - 1
            # Step-specific aux feature names
            hexo_names_step = [n.replace('_hh', f'_{h}h') for n in self.horizon_exogenous_list]
            step_feature_names = hexo_names_step + ['pos_sin', 'pos_cos', 'pm0_pm2_5_scaled', 'pm0_pm10_scaled']
            # Background/explain sets for this step
            X_step_bg = X_aux_bg[:, step_idx, :]
            X_step_ex = X_aux_ex[:, step_idx, :]
            # Fix a reference context (encoder + full aux) from validation
            ref_idx = 0
            enc_ref = X_enc_val[ref_idx]
            aux_ref = X_aux_val[ref_idx]
            def f_step(X_step_in: np.ndarray, target_idx: int) -> np.ndarray:
                n = X_step_in.shape[0]
                enc = np.repeat(enc_ref[None, :, :], repeats=n, axis=0)
                aux = np.repeat(aux_ref[None, :, :], repeats=n, axis=0)
                aux[:, step_idx, :] = X_step_in
                preds = self.model.predict([enc, aux], verbose=0)
                return preds[:, step_idx, target_idx]
            for target_idx, tgt_name in enumerate(['pm2_5', 'pm10']):
                try:
                    explainer = shap.KernelExplainer(lambda x: f_step(x, target_idx), X_step_bg)
                    shap_vals = explainer.shap_values(X_step_ex, nsamples=100)
                    # Beeswarm
                    try:
                        shap.summary_plot(shap_vals, X_step_ex, feature_names=step_feature_names, show=False)
                        plt.title(f"SHAP Beeswarm (LSTM) – {tag} h={h} {tgt_name}")
                        plt.tight_layout()
                        plt.savefig(os.path.join(out_dir, f"{tag}_{tgt_name}_h{h}_beeswarm.png"), dpi=150, bbox_inches='tight')
                        plt.close()
                    except Exception as e:
                        print(f"⚠️ SHAP beeswarm failed for h={h}, {tgt_name}: {e}")
                    # Bar
                    try:
                        shap.summary_plot(shap_vals, X_step_ex, feature_names=step_feature_names, show=False, plot_type='bar')
                        plt.title(f"SHAP Bar (LSTM) – {tag} h={h} {tgt_name}")
                        plt.tight_layout()
                        plt.savefig(os.path.join(out_dir, f"{tag}_{tgt_name}_h{h}_bar.png"), dpi=150, bbox_inches='tight')
                        plt.close()
                    except Exception as e:
                        print(f"⚠️ SHAP bar failed for h={h}, {tgt_name}: {e}")
                except Exception as e:
                    print(f"⚠️ SHAP explain failed for h={h}, {tgt_name}: {e}")
    ...
    def save_artifacts(self, out_dir_name: str = 'direct_lstm_multi_horizon') -> str:
        ...
        joblib.dump({'pm': self.pm_scaler, 'weather': self.weather_scaler, 'pollutant': self.pollutant_scaler, 'interaction': self.interaction_scaler}, os.path.join(out_dir, f'{out_dir_name}_scalers.pkl'))
