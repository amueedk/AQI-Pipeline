...
# Horizons to train
HORIZONS = [1,2,6,12,24,48,72]

print(f"📋 Configuration:")
print(f"   Horizons: {HORIZONS}")
print(f"   Model: Multi-Output RandomForest (n_estimators={MODEL_CONFIG['n_estimators']})")
...
class RandomForestMultiOutputTrainer:
    """
    Multi-Output RandomForest training pipeline for AQI forecasting
    ...
    def train_multi_output_models(self, df: pd.DataFrame, horizons: List[int]) -> Dict:
        """
        Train multi-output RandomForest models
        """
        print(f"\n🌳 Training Multi-Output RandomForest models for all horizons: {horizons}")
        ...
        rf_pm2_5 = MultiOutputRegressor(base_rf)
        rf_pm10 = MultiOutputRegressor(base_rf)

        # Train PM2.5 model
        print(f"   Training Multi-Output PM2.5 model...")
        rf_pm2_5.fit(X_train, y_pm2_5_train)
        ...
        # Evaluate models
        pm2_5_pred = rf_pm2_5.predict(X_test)
        pm10_pred = rf_pm10.predict(X_test)
        ...
        for i, horizon in enumerate(horizons):
            ...
            pm2_5_train_pred = rf_pm2_5.predict(X_train)
            pm10_train_pred = rf_pm10.predict(X_train)
        ...
        # Store models
        self.models['pm2_5_multi_output'] = rf_pm2_5
        self.models['pm10_multi_output'] = rf_pm10
    ...
    def validate_data_alignment(self, X_train: pd.DataFrame, X_test: pd.DataFrame,
        ...
        """
        Validate critical data alignment issues for Multi-Output RandomForest
        """
        print(f"🔍 Validating data alignment for Multi-Output RandomForest...")
        ...
        # Check 2: Data ranges (should be in original scale for RF)
...
print("✅ RandomForestMultiOutputTrainer class defined!")

# Initialize and train the model
print(f"🌳 Initializing Multi-Output RandomForest Trainer")
trainer = RandomForestMultiOutputTrainer(MODEL_CONFIG)
...
ax1.set_title('PM2.5 RMSE by Horizon (Multi-Output RandomForest)')
...
ax2.set_title('PM2.5 R² by Horizon (Multi-Output RandomForest)')
...
ax3.set_title('PM10 RMSE by Horizon (Multi-Output RandomForest)')
...
ax4.set_title('PM10 R² by Horizon (Multi-Output RandomForest)')
...
# Get feature importance from first estimator (all estimators have same importance in RF)
...
ax.set_title('Multi-Output RandomForest - Top 15 Features')
ax.legend()
...
print("\n" + "=" * 80)
print("✅ Multi-Output RandomForest analysis complete!") 