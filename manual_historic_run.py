...
def collect_historical_data_june_2025():
    """
    Collect historical data for June 16-30, 2025 with timestamp matching.
    Uses multiple API calls to work around record limits.
    Returns DataFrame with matched weather and pollution data.
    """
    logger.info("Collecting historical data for June 16-30, 2025...")
    
    # Define date range: June 16, 2025 00:00 UTC to June 30, 2025 23:59 UTC
    start_date = datetime(2025, 6, 16, 0, 0, 0, tzinfo=None)  # June 16, 2025 00:00
    end_date = datetime(2025, 6, 30, 23, 59, 59, tzinfo=None)  # June 30, 2025 23:59
...
def run_manual_backfill_csv_and_hopsworks():
    ...
    logger.info("STEP 2: Collecting historical data for June 16-30, 2025...")
    raw_df = collect_historical_data_june_2025()
    if raw_df.empty:
    ...
    raw_data_path = "data/raw_historical_data_june16_30_2025.csv"
    ...
    engineered_data_path = "data/engineered_features_june16_30_2025.csv"
    ...
    try:
        ...
        commit_message = f"Add historical data for June 16-30, 2025 ({len(engineered_df)} records)"
    ...
    success = uploader.push_features(
        ...
        description="Historical backfill of PM2.5 and PM10 prediction features for Multan (June 16-30, 2025). Targets: pm2_5, pm10 (raw concentrations), Reference: us_aqi (final AQI)."
    )
